...
const themes_1 = require("baseui/themes");
...
function isAcceptedColorValue(color) {
    if (!color)
        return false;
...
exports.default = (context) => {
    ...
    let themePropsDecorationList = [];
    // Create a decorator type that we use to decorate theme colors
    function getColorDecorationType(coloringStyle, colorVal) {
        const decorationType = vscode.window.createTextEditorDecorationType(Object.assign({ borderWidth: coloringStyle === 'border'
                ? '2px'
                : coloringStyle === 'underline'
                    ? '0 0 2px'
                    : '0', borderStyle: 'solid', borderColor: colorVal, backgroundColor: coloringStyle === 'background' ? colorVal : 'transparent' }, (coloringStyle === 'background' && colorVal.startsWith('#')
            ? { color: getContrastYIQ(colorVal) }
            : {})));
        return decorationType;
    }
    // Memoize decorator types per color value and current coloringStyle setting
    function getMemoizedDecorationType(coloringStyle, colorVal) {
    ...
    // Unset decorations
    function undecorate(activeEditor) {
        const keys = [];
        for (const prop in decorationTypeMap) {
            if (decorationTypeMap.hasOwnProperty(prop)) {
                keys.push(prop);
    ...
    function updateDecorations() {
        ...
        const theme = themeMode === 'Light' ? themes_1.LightTheme : themes_1.DarkTheme;
        ...
        while ((match = regEx.exec(text))) {
            // @ts-ignore
            const themeColorVal = theme.colors[match[3]];
            ...
            const colorVal = isAcceptedColorValue(themeColorVal)
                ? themeColorVal
                : 'transparent';
            // Start position excluding the `colors.` part
            const startPos = activeEditor.document.positionAt(match.index + match[1].length + match[2].length);
        ...
        for (let i = 0; i < themeProps.length; i++) {
            ...
            while ((match = regEx.exec(text))) {
                // @ts-ignore
                const themePropVal = theme[themeProps[i]][match[3]];
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
        if (event.affectsConfiguration('baseweb.theme.coloring.enabled') ||
