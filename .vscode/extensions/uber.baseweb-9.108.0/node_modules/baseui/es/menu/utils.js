...
export function scrollItemIntoView(child, parent, isFirst, isLast, scrollAllignInView = 'auto') {
  if (!child) return;
  ...
  if (childRect.bottom > parentRect.bottom) {
    if (isLast) {
      ...
      parent.scrollTop = targetBottom - (scrollAllignInView === 'center' ? Math.round((parentRect.height + childRect.height) / 2) : parentRect.height);
    ...
    if (isFirst) {
      ...
      parent.scrollTop = child.offsetTop - (scrollAllignInView === 'center' ? Math.round((parentRect.height - childRect.height) / 2) : 0);
    }
  }
}